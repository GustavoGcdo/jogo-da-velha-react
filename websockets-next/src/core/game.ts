...
export class Game {
  ...
  mark(quadrant: [number, number]): boolean {
  ...
  move(oldPosition: Position, newPosition: Position): boolean {
    if (this.gameSpaces[newPosition.row][newPosition.collumn] != '') return false;
    if (this.gameSpaces[oldPosition.row][oldPosition.collumn] != this.currentPlayer) return false;
    if (oldPosition.row == newPosition.row && oldPosition.collumn == newPosition.collumn)
      return false;
  ...
  setExplicitWinner(player: GameSymbol) {
    this.explicitWinner = player;
  }
